...
int Interpreter::evaluateExpression(string& expression) {
    ...
    for (const auto& expr : expressions) {
            ...
            for (const auto& token : tokens) {
                if (isVariable(token, variables_int->variablesInteger)) {
                    variableName = token;
                    result = variables_int->getVariableInteger(variableName);
...
void Interpreter::DivisionIntoMethodsAndTokens(string line)
{
    ...
    if (pos != string::npos)
    {
        ...
        variables_int->setVariablesInteger(resultViariableName, evaluateExpression(Method));
    }
...
void Interpreter::ReadTokensFunction(string line)
{
    ...
    {
        variables_int->setVariablesInteger(LineBetweenTokens("int ", " =", line), stoi(LineBetweenTokens(" = ", ";", line)));
    }
    else if (LineContainsWord("print", line))
    {
        temp = LineBetweenTokens("(", ")", line);
        cout << variables_int->getVariableInteger(temp) << endl;
    }
